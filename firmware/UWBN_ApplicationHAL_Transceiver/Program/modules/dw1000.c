...
void DWT_ReadData( uint16_t regAddr, uint16_t subIndex, void *readBuf, uint32_t lens )
{
  ...
  DW1000_CSD_L();
  SPI_SendData(dw1000.spi->handle, header, count, HAL_MAX_DELAY);
  SPI_RecvData(dw1000.spi->handle, readBuf, lens, HAL_MAX_DELAY);
  DW1000_CSD_H();

#else
  DW1000_CSD_L();
  for (uint32_t i = 0; i < count; i++) {
    SPI_RW(dw1000.spi->handle, header[i]);
  }
  for (uint32_t i = 0; i < lens; i++) {
    ((uint8_t*)readBuf)[i] = SPI_RW(dw1000.spi->handle, 0x00);
  }
  DW1000_CSD_H();
...
void DWT_WriteData( uint16_t regAddr, uint16_t subIndex, const void *writeBuf, uint32_t lens )
{
  ...
  DW1000_CSD_L();
  SPI_SendData(dw1000.spi->handle, header, count, HAL_MAX_DELAY);
  SPI_SendData(dw1000.spi->handle, writeBuf, lens, HAL_MAX_DELAY);
  DW1000_CSD_H();
  ...
  for (uint32_t i = 0; i < lens; i++) {
    SPI_RW(dw1000.spi->handle, ((uint8_t*)writeBuf)[i]);
  }
  DW1000_CSD_H();
...
{
  SPI_SetSpeed(dw1000.spi->handle, DW1000_SPIx_SPEED_LOW);
}
...
{
  SPI_SetSpeed(dw1000.spi->handle, DW1000_SPIx_SPEED_HIGH);
}
...
//  HAL_NVIC_EnableIRQ(DW1000_IRQn);
  ...
  /* SPI Init ****************************************************************/
  dw1000.spi->handle->Instance               = DW1000_SPIx;
  dw1000.spi->handle->Init.Mode              = SPI_MODE_MASTER;
  dw1000.spi->handle->Init.Direction         = SPI_DIRECTION_2LINES;
  dw1000.spi->handle->Init.DataSize          = SPI_DATASIZE_8BIT;
  dw1000.spi->handle->Init.CLKPolarity       = SPI_POLARITY_LOW;
  dw1000.spi->handle->Init.CLKPhase          = SPI_PHASE_1EDGE;
  dw1000.spi->handle->Init.NSS               = SPI_NSS_SOFT;
  dw1000.spi->handle->Init.BaudRatePrescaler = DW1000_SPIx_SPEED_LOW;
  dw1000.spi->handle->Init.FirstBit          = SPI_FIRSTBIT_MSB;
  dw1000.spi->handle->Init.TIMode            = SPI_TIMODE_DISABLE;
  dw1000.spi->handle->Init.CRCCalculation    = SPI_CRCCALCULATION_DISABLE;
  dw1000.spi->handle->Init.CRCPolynomial     = 7;
  HAL_SPI_Init(dw1000.spi->handle);
}
...
int8_t DWT_Initialise( DWT_ConfigTypeDef *config )
{
  ...
  dw1000.irqEventCallback = DWT_ISR;
 ...
 * @retval None
 */
uint64_t DWT_ReadRxTimestamp64( void )
{
  uint8_t buff[5];
  uint64_t timestamp = 0;

  // Get the adjusted time of arrival
  DWT_ReadData(DW1000_RX_TIME, 0x00, buff, 5);
...
//  mask = (enable) ? mask | bitmask : mask & ~bitmask;
  if (enable) {
    mask |= bitmask;
  }
  else {
