...
uint32_t IAP_Download( uint8_t *binaryFile )
{
  ...
  Serial_RecvData(fileInfo, 16, HAL_MAX_DELAY);
  if ((fileInfo[0] == 'K') && (fileInfo[1] == 'S') && (fileInfo[2] == 'U') && (fileInfo[3] == 'L')) {
    ...
    if (crc == *recvCRC) {
      ...
      while (quotient--) {
        Serial_RecvData(binaryFile + offset, *recvLens, HAL_MAX_DELAY);
        offset += *recvLens;
        LED_G_Toggle();
      }
      Serial_RecvData(binaryFile + offset, remainder, HAL_MAX_DELAY);
    }
  }
